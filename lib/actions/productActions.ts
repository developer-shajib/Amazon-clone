...
// export async function getAllProductsForAdmin({ query, page = 1, sort = 'latest', limit }: { query: string; page?: number; sort?: string; limit?: number }) {
//   await mongoDBConnect();
...
//   const order: Record<string, 1 | -1> = sort === 'best-selling' ? { numSales: -1 } : sort === 'price-low-to-high' ? { price: 1 } : sort === 'price-high-to-low' ? { price: -1 } : sort === 'avg-customer-review' ? { avgRating: -1 } : { _id: -1 };
...
// // GET ALL PRODUCTS
// export async function getAllProducts({ query, limit, page, category, tag, price, rating, sort }: { query: string; category: string; tag: string; limit?: number; page: number; price?: string; rating?: string; sort?: string }) {
//   const {
//     common: { pageSize }
//   } = await getSetting();
//   limit = limit || pageSize;
...
//   const tags = await Product.aggregate([{ $unwind: '$tags' }, { $group: { _id: null, uniqueTags: { $addToSet: '$tags' } } }, { $project: { _id: 0, uniqueTags: 1 } }]);
//   return (
//     (tags[0]?.uniqueTags
